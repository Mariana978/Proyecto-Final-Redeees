...
| `config\_clienteX.json` | Configuración de cada cliente (solo esto cambia entre PCs) |
...
cd Desktop/proyecto\_sdn/prueba\_un\_pc
...
python cliente.py config\_cliente1.json

python cliente.py config\_cliente2.json

python cliente.py config\_cliente3.json
...
| Permitir UDP | Regla: protocolo=UDP, puerto=9001, acción=PERMITIR, prioridad=5 |

| Bloquear UDP | Regla: protocolo=UDP, puerto=9001, acción=BLOQUEAR, prioridad=10 |

| Bloquear por IP | Regla: ip\_origen=127.0.0.1, acción=BLOQUEAR, prioridad=5 |

| Reportar al controlador | Regla: acción=REPORTAR, prioridad=20 |

| Conflicto de prioridad | Dos reglas mismo flujo, distinta prioridad — gana la mayor |
